...
export class VoxelImager {
    ...
    constructor(scene) {
        ...
        const rtt = new RenderTargetTexture("Voxel Imager RTT", {
        ...
        rtt.renderList = [];
        const camera = new ArcRotateCamera("Voxel Imager Camera", -Math.PI / 4, Math.PI / 2.5, 2, new Vector3(0, 0, 0), scene, false);
        this._camera = camera;
    ...
    async createImageFromMesh(mesh) {
        if (!this._rtt.isReadyForRendering())
            await this._waitTillReady();
        const rtt = this._rtt;
        ...
        const imageData = new ImageData(new Uint8ClampedArray(rawPixels.buffer), this._imageSize, this._imageSize);
