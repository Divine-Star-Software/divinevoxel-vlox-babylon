...
export class ImageArrayTexture extends Texture {
    ...
    constructor(imgs, scene) {
        ...
        if (imgs) {
            ...
            gl.texStorage3D(gl.TEXTURE_2D_ARRAY, Math.log2(Math.max(width, height)) + 1, gl.RGBA8, width, height, layers);
            for (let layer = 0; layer < imgs.length; layer++) {
                const img = imgs[layer];
                gl.texSubImage3D(gl.TEXTURE_2D_ARRAY, 0, // Mipmap level
                0, 0, layer, width, height, 1, // depth
            ...
            const itex = new InternalTexture(scene.getEngine(), InternalTextureSource.Unknown);
            itex.width = width;
            itex.height = height;
            itex.isReady = true;
