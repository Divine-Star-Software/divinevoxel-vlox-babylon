    ...
    updateVertexData(section, data) {
        ...
        for (let i = 0; i < totalMeshes; i++) {
            ...
            if (section.meshes.has(subMeshMaterial)) {
                mesh = section.meshes.get(subMeshMaterial);
            }
            else {
                if (!DVEBRSectionMeshes.meshCache.length) {
                    ...
                    const geometry = new Geometry(Geometry.RandomId(), this.scene, undefined, false, newMesh);
                    geometry._boundingInfo = new BoundingInfo(new Vector3(0, 0, 0), new Vector3(0, 0, 0));
            ...
            if (mesh.metadata.buffer && mesh.metadata.buffer instanceof Buffer) {
                const buffer = mesh.metadata.buffer;
                for (const bufferKind in mesh.getVerticesDataKinds()) {
            ...
            mesh.metadata.buffer = DVEBRMesh.UpdateVertexDataBuffers(mesh, this.engine, meshData.verticies, meshData.indices);
