...
class VoxelGeometryIntersectResult {
    ...
    uv;
    triangleId;
    constructor(hit, normal, position, uv, triangleId) {
        this.hit = hit;
        this.normal = normal;
...
class VoxelMeshIntersectResult {
    ...
    error;
    triangle;
    constructor(found, foundObject, t, error, triangle) {
        this.found = found;
...
function BVH_TriangleIntersect(v0, v1, v2, rayOrigin, rayDirection) {
...
function VoxelGeometryIntersect(ro, rd, nodeId, voxel_indice, vertices, indices) {
    vertexCursor.data = vertices;
    let intersectResult = new VoxelGeometryIntersectResult(false, new Vector3(0, 0, 0), new Vector3(0, 0, 0), new Vector2(0, 0), 0);
    ...
    let indiceOffsets = new Vector2(voxel_indice[voxelIndiceOffset + nodeId * 2], voxel_indice[voxelIndiceOffset + nodeId * 2 + 1]);
    ...
    for (let indiceIndex = 0; indiceIndex < length; indiceIndex += 3) {
        console.log(indices[indiceOffset + indiceIndex], indices[indiceOffset + indiceIndex + 1], indices[indiceOffset + indiceIndex + 2]);
        let v1 = vertexOffset + indices[indiceOffset + indiceIndex];
        vertexCursor.index = v1;
        v1Position.set(vertexCursor.positionX, vertexCursor.positionY, vertexCursor.positionZ);
        let v2 = vertexOffset + indices[indiceOffset + indiceIndex + 1];
        vertexCursor.index = v2;
        v2Position.set(vertexCursor.positionX, vertexCursor.positionY, vertexCursor.positionZ);
        let v3 = vertexOffset + indices[indiceOffset + indiceIndex + 2];
        vertexCursor.index = v3;
        v3Position.set(vertexCursor.positionX, vertexCursor.positionY, vertexCursor.positionZ);
        let triResult = BVH_TriangleIntersect(v2Position, v1Position, v3Position, ro, rd);
        console.log(v1Position.toString(), v2Position.toString(), v3Position.toString(), triResult);
        if (triResult.t < t) {
    ...
    if (intersectResult.hit) {
        ...
        vertexCursor.index = v1;
        v1Normal.set(vertexCursor.normalX, vertexCursor.normalY, vertexCursor.normalZ);
        let v2 = vertices[vertexOffset + indices[finalIndex]];
        vertexCursor.index = v2;
        v2Normal.set(vertexCursor.normalX, vertexCursor.normalY, vertexCursor.normalZ);
        let v3 = vertices[vertexOffset + indices[finalIndex]];
        vertexCursor.index = v3;
        v3Normal.set(vertexCursor.normalX, vertexCursor.normalY, vertexCursor.normalZ);
...
const STACK_SIZE = 13;
const stack = Array.from({ length: STACK_SIZE }, () => new StackNode(0, 0));
const makeMesh = (struct, hit = false, parent = false) => {
    const mesh = parent
    ...
    mesh.scaling.set(struct.maxX - struct.minX, struct.maxY - struct.minY, struct.maxZ - struct.minZ);
    mesh.position.set(struct.minX + mesh.scaling.x / 2, struct.minY + mesh.scaling.y / 2, struct.minZ + mesh.scaling.z / 2);
    mesh.material;
    return mesh;
};
const VOXEL_NODE_INDEX = 4095;
function VoxelMeshIntersect(ro, rd, voxel_bvh, voxel_indice_offsets, mesh_verterices, mesh_indices) {
    ...
    let intersectResult = new VoxelMeshIntersectResult(false, 0, 0, false, new VoxelGeometryIntersectResult(false, new Vector3(0, 0, 0), new Vector3(0, 0, 0), new Vector2(0, 0), 0));
    const mehses = [];
    let stackIndex = 0;
    ...
    const inverseDir = new Vector3(rd.x !== 0 ? 1 / rd.x : 0, rd.y !== 0 ? 1 / rd.y : 0, rd.z !== 0 ? 1 / rd.z : 0);
    do {
        currentNode.setIndex(currentNodeIndex);
        // Check if current node is a leaf
        if (currentNode.nodeType == 2.0) {
            // Found a leaf node check against actual triangles
            let geomtryResult = VoxelGeometryIntersect(ro, rd, currentNodeIndex - VOXEL_NODE_INDEX, voxel_indice_offsets, mesh_verterices, mesh_indices);
...
export class BVHViewer {
    ...
    static _material;
    static _hitMaterial;
    static _parentMaterial;
    static instances = new Set();
    constructor(mesh, scene, tree, treeIndices, vertices, indices) {
        this.mesh = mesh;
        this.scene = scene;
    ...
    testIntersection(rayOrigin, rayDirectoin) {
        return VoxelMeshIntersect(rayOrigin, rayDirectoin, this.tool.tree, this.tool.indices, this.vertices, this.indices);
    }
    createBoxes(level, ro, rd) {
        for (const mesh of this._boxes) {
            mesh.dispose();
        }
        const minBox = new Vector3();
        ...
        if (level == 0) {
            ...
            for (let i = 0; i < levelSize; i++) {
                ...
                if (ro && rd) {
                    ...
                    const t = BoundingBoxIntersect(minBox, maxBox, ro, new Vector3(rd.x !== 0 ? 1 / rd.x : 0, rd.y !== 0 ? 1 / rd.y : 0, rd.z !== 0 ? 1 / rd.z : 0));
                    if (t != Infinity) {
