...
export class VoxelPathSegmentMesh extends Mesh {
    ...
    _dispose;
    _dirty = false;
    constructor(pathMesh, path, segment) {
        ...
        const geometry = new Geometry(Geometry.RandomId(), this.pathMesh.scene, undefined, true, this);
    ...
    update() {
        ...
        for (const face of faceIndices) {
            indices.push(indexOffset + face[0], indexOffset + face[1], indexOffset + face[2], indexOffset + face[0], indexOffset + face[2], indexOffset + face[3]);
        }
    }
    build() {
        if (!this._dirty)
...
export class VoxelPathMesh {
    ...
    constructor(scene, name = "") {
        ...
        this._material = VoxelPathMesh.Materials.get(scene).clone(`Voxel Line Mesh ${name}`);
        this.setColor(1, 1, 1, 0.8);
    }
    setColor(r, g, b, a = this.color.a) {
    ...
    setPath(path) {
        ...
        for (let i = 0; i < path.segments.length; i++) {
            this.segments.push(new VoxelPathSegmentMesh(this, path, path.segments[i]));
        }
        const segmentAdded = this.path.createEventListener("segmentAdded", ({ detail: segment }) => {
            this.segments.push(new VoxelPathSegmentMesh(this, path, segment));
        });
        this.path.addEventListener("segmentAdded", segmentAdded);
        const segmentRemoved = this.path.createEventListener("segmentRemoved", ({ detail: segment }) => {
