...
class PositionAxes {
    ...
    min;
    max;
    constructor(controls, normal, axex, _states) {
        ...
        mesh.setVerticesBuffer(new VertexBuffer(this.controls.scene.getEngine(), colors, "axes", false, undefined, 1));
        ...
        this.mesh.position.set(normal.x > 0 ? normal.x / 2 : 0, normal.y > 0 ? normal.y / 2 : 0, normal.z > 0 ? normal.z / 2 : 0);
    ...
    deltas = new Vector3();
    update(ray, mouseDown) {
        min.set(this.min.x + this.mesh.position.x + this.controls.parent.position.x, this.min.y + this.mesh.position.y + this.controls.parent.position.y, this.min.z + this.mesh.position.z + this.controls.parent.position.z);
        max.set(this.max.x + this.mesh.position.x + this.controls.parent.position.x, this.max.y + this.mesh.position.y + this.controls.parent.position.y, this.max.z + this.mesh.position.z + this.controls.parent.position.z);
        ...
        if (this.active) {
            ...
            const planeNormal = Vector3.Cross(Vector3.Cross(axis, ray.direction), axis).normalize();
            // Define a new drag plane every frame (or store it if performance becomes an issue)
            const dragPlane = Plane.FromPositionAndNormal(this.deltaPoint, planeNormal);
            const distance = ray.intersectsPlane(dragPlane);
            if (!distance)
                return;
            const intersectionPoint = new Vector3(ray.origin.x + ray.direction.x * distance, ray.origin.y + ray.direction.y * distance, ray.origin.z + ray.direction.z * distance);
            ...
            const needUpdate = dx !== this.deltas.x || dy !== this.deltas.y || dz !== this.deltas.z;
            if (needUpdate) {
        ...
        if (ray.intersectsBoxMinMax(min, max)) {
            if (mouseDown && !this.active && !this.controls._controlActive) {
                ...
                this.deltaPoint.set(this.controls.parent.position.x, this.controls.parent.position.y, this.controls.parent.position.z);
...
export class VoxelControls extends TypedEventTarget {
    scene;
    ...
    constructor(scene) {
        ...
        this.xAxes = new PositionAxes(this, new Vector3(1, 0, 0), Axes.X, this._states);
        this.yAxes = new PositionAxes(this, new Vector3(0, 1, 0), Axes.Y, this._states);
        this.zAxes = new PositionAxes(this, new Vector3(0, 0, 1), Axes.Z, this._states);
        this.updateColors(new Color3(1, 0, 0), new Color3(0, 1, 0), new Color3(0, 0, 1));
        this.material.setArray4("states", this._states);
    }
    updateColors(x, y, z) {
        if (!this.colors)
            this.colors = {};
    ...
    update(mouseDown, rayOrigin, rayDirection, length) {
        this.updatePosition();
