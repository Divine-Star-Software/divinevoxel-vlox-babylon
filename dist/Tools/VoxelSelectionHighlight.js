...
const tempP1 = [0, 0, 0];
const tempP2 = [0, 0, 0];
const addVoxelFace = (face, outlineAll, selection, size, x, y, z, mesh) => {
    ...
    for (let i = 0; i < facePoints.length; i++) {
        ...
        if (!outlineAll) {
            if (selection.isSelected(x + faceChecks[i][0], y + faceChecks[i][1], z + faceChecks[i][2]) &&
                !selection.isSelected(x + faceChecks[i][0] + normal[0], y + faceChecks[i][1] + normal[1], z + faceChecks[i][2] + normal[2])) {
                continue;
            }
        }
                ...
                for (let z = oz; z < ez; z++) {
                    ...
                    for (let face = 0; face < 6; face++) {
                        ...
                        if (!selection.isSelected(nx, ny, nz)) {
                            addVoxelFace(face, this.outlineAll, selection, 1.01, x, y, z, this.mesh);
                        }
                    }
                }
            }
