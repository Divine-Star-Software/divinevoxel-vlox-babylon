...
async function loadAndInvertImage(imgSrcData) {
    if (!context)
        throw new Error("");
...
const buildMesh = (scene, voxelData, voxelId, stateID) => {
    const meshedVoxel = MeshVoxel(voxelData);
    if (!meshedVoxel)
        return false;
    if (meshedVoxel[0][0] == 1)
        throw new Error(`Not in right mode`);
    ...
    VoxelModelIndex.registerModel(voxelId, stateID, meshedVoxel[0], renderedMaterial, material._material);
    DVEBRMesh.UpdateVertexData(mesh, scene.getEngine(), meshedVoxel[0][1][0]);
    return mesh;
};
export default async function CreateDisplayIndex(data, getProgress) {
    ...
    if (!CacheManager.cacheStoreEnabled &&
        ...
        for (const [voxelId, states] of VoxelModelIndex.voxelModels) {
            for (const [stateId, state] of states) {
                const material = DVEBabylonRenderer.instance.materials.get(state.material);
                if (!material)
                    continue;
        ...
        for (const [stateId, state] of states.states) {
            ...
            if (state.data.display.type == "model") {
                ...
                if (state.data.display.state) {
                    addVoxelData.state = voxelSchema.state.readString(state.data.display.state);
                }
                ...
                const mesh = buildMesh(voxelImager.scene, rawData, voxelId, state.data.id);
                if (!mesh)
                    console.error(`Could not make mesh for ${voxelId}`, rawData);
                if (!mesh)
                    continue;
            ...
            if (state.data.display.type == "texture") {
                ...
                const path = await loadAndInvertImage(TextureManager.getTexture(textureType || "dve_voxel").getTexturePath(source));
