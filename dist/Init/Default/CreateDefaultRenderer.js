...
export async function CreateTextures(scene, textureData, progress) {
    ...
    for (const [key, type] of TextureManager._compiledTextures) {
        ...
        animatedTexture.shaderTexture = new RawTexture(animatedTexture._buffer, animatedTexture._size, animatedTexture._size, Engine.TEXTUREFORMAT_RED_INTEGER, null, false, undefined, Engine.TEXTURE_NEAREST_NEAREST, Engine.TEXTURETYPE_UNSIGNED_SHORT);
        animatedTexture.shaderTexture.metadata = {
...
export async function CreateDefaultRenderer(initData) {
    ...
    const DefaultSubstances = defaultSubstances.map((id) => {
        return {
            id,
            material: {
                alphaBlending: id == "dve_liquid" ||
                    id == "dve_transparent" ||
                    id == "dve_flora_transparent"
                    ? true
                    : false,
                alphaTesting: id != "dve_liquid" &&
                    id != "dve_transparent" &&
                    id != "dve_flora_transparent"
                    ? false
                    : true,
    ...
    for (const mat of materials) {
        renderer.materials.register(mat.id, initData.createMaterial(renderer, scene, mat));
    }
    let time = 0;
    ...
    scene.registerBeforeRender(() => {
        ...
        for (const [key, type] of TextureManager._compiledTextures) {
            if (type.animatedTexture.tick(scene.deltaTime)) {
                type.animatedTexture.shaderTexture.update(type.animatedTexture._buffer);
            }
        }
