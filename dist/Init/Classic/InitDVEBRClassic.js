...
import { CreateDefaultRenderer, CreateTextures, } from "../Default/CreateDefaultRenderer";
...
export default async function InitDVEBRClassic(initData) {
    const progress = new WorkItemProgress();
    if (initData.getProgress)
        initData.getProgress(progress);
    progress.startTask("Init Classic Renderer");
    await CreateTextures(initData.scene, initData.textureData, progress);
    DVEBRShaderStore.setShaderData("dve_voxel_particle", [
        "world",
        "viewProjection",
        "dve_voxel",
        "dve_voxel_animation",
    ...
    DVEBRShaderStore.storeShader("dve_voxel_particle", "vertex", VoxelParticleShader.GetVertex());
    DVEBRShaderStore.storeShader("dve_voxel_particle", "frag", VoxelParticleShader.GetFragment());
    for (const material of defaultMaterials) {
        DVEBRShaderStore.setShaderData(material, [
            "world",
            "viewProjection",
            "worldOrigin",
            "cameraPosition",
        ...
        DVEBRShaderStore.storeShader(material, "vertex", VoxelBaseShader.GetVertex({
            doAO: true,
        }));
        DVEBRShaderStore.storeShader(material, "frag", material.includes("liquid")
            ? VoxelBaseShader.GetFragment(VoxelBaseShader.DefaultLiquidFragmentMain(true))
    ...
    const renderer = await CreateDefaultRenderer({
        ...
        createMaterial: (renderer, scene, matData) => {
            const newMat = new DVEBRClassicMaterial(renderer.voxelScene.options, matData.id, {
                scene,
                data: {
