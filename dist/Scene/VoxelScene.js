...
export class VoxelScene {
    ...
    init(scene) {
        ...
        for (let i = 0; i < VoxelPalettesRegister.material._palette.length; i++) {
            multiMaterial.subMaterials.push(this.renderer.materials.get(VoxelPalettesRegister.material._palette[i])
                ._material);
        }
    ...
    updateMesh(subBufferMesh, data) {
        const bufferMesh = subBufferMesh.allocation._bufferMesh;
        if (data.vertexCount > subBufferMesh.verticesCount ||
        ...
        bufferMesh.writeBuffers(subBufferMesh.allocation, data.verticies, data.indices);
        return subBufferMesh;
    }
    addMesh(data, x, y, z) {
        let allocation = null;
        for (let i = 0; i < this._meshBuffers.length; i++) {
            allocation = this._meshBuffers[i].allocate(data.vertexCount, data.indexCount);
            if (allocation)
                break;
        }
        if (!allocation) {
            allocation = this._addBufferMesh().allocate(data.vertexCount, data.indexCount);
            if (!allocation) {
        ...
        const totalVertices = data.verticies.length / VoxelMeshVertexStructCursor.VertexFloatSize;
        ...
        subBufferMesh.mesh = new SubMesh(data.material, subBufferMesh.verticesStart, subBufferMesh.verticesCount, subBufferMesh.indicesStart, subBufferMesh.indicesCount, bufferMesh, undefined, false
        // false
        );
        const scene = bufferMesh.voxelScene.renderer.scene;
        subBufferMesh.transform = new TransformNode("", scene);
        subBufferMesh.transform.position.set(x, y, z);
        subBufferMesh.transform.computeWorldMatrix();
        for (let i = 0; i < bufferMesh.voxelScene.renderer.scene.rootNodes.length; i++) {
    ...
    beforRender() {
        ...
        for (const [, dimension] of MeshRegister._dimensions) {
            for (const [, sector] of dimension) {
                ...
                max.set(sector.position[0] + WorldSpaces.sector.bounds.x, sector.position[1] + WorldSpaces.sector.bounds.y, sector.position[2] + WorldSpaces.sector.bounds.z);
