...
export class VoxelExplodeParticles {
    ...
    constructor(scene, dataCursor, _count = 15, disposeOnDead = true) {
        ...
        const material = DVEBabylonRenderer.instance.materials.get("dve_voxel_particle");
        ...
        this._sps.updateParticle = (particle) => {
            ...
            const ligtvoxel = this.dataCursor.getVoxel(Math.floor(this._mesh.position.x + temp.x), Math.floor(this._mesh.position.y + temp.y), Math.floor(this._mesh.position.z + temp.z));
            if (ligtvoxel) {
            ...
            if (!this.dataCursor
                .getVoxel(Math.floor(this._mesh.position.x + temp.x), Math.floor(this._mesh.position.y + temp.y - particle.scale.y / 2), Math.floor(this._mesh.position.z + temp.z))
                ?.isAir()) {
