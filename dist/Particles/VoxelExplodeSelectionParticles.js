...
export class VoxelExplodeSelectionParticles {
    ...
    _count;
    disposeOnDead;
    static CreateParticleData(iSelection, cursor) {
        ...
        const textures = new Array(size.x * size.y * size.z).fill(0);
        for (let x = ox; x < ex; x++) {
            for (let y = oy; y < ey; y++) {
                for (let z = oz; z < ez; z++) {
                    const index = GetXYZOrderArrayIndex(x - ox, y - oy, z - oz, size.x, size.y, size.z);
                    if (!iSelection.isSelected(x, y, z))
                        continue;
                    ...
                    const modState = SchemaRegister.getVoxelSchemas(voxel.getStringId())
                        ...
                        .toStateString();
                    textures[index] =
                        VoxelPropertiesRegister.VoxelProperties[voxel.getVoxelId()]["dve_particle_data"]?.[modState] || "";
                }
            }
        }
        return textures;
    }
    static Quads = new Map();
    _sps;
    _mesh;
    _dead = 0;
    _disposed = false;
    ...
    _beforeRender;
    _material;
    constructor(scene, dataCursor, _count = 15, disposeOnDead = true) {
        this.scene = scene;
        this.dataCursor = dataCursor;
        this._count = _count;
        this.disposeOnDead = disposeOnDead;
        this._material =
            DVEBabylonRenderer.instance.materials.get("dve_voxel_particle")._material;
        ...
        this._sps.initParticles = () => {
            for (let i = 0; i < this._sps.nbParticles; i++) {
                ...
                particle.scale.set(0.05 + sizeX * 0.1 * 0.2, 0.05 + sizeY * 0.1 * 0.2, 1);
        ...
        this._sps.updateParticle = (particle) => {
            ...
            const ligtvoxel = this.dataCursor.getVoxel(Math.floor(temp.x), Math.floor(temp.y), Math.floor(temp.z));
            if (ligtvoxel) {
                const light = ligtvoxel.getLight();
                if (light >= 0)
                    particle.color.b = light;
            }
            if (!this.dataCursor
                .getVoxel(Math.floor(temp.x), Math.floor(temp.y - particle.scale.y / 2), Math.floor(temp.z))
                ?.isAir()) {
    ...
    explode(iSelection, textures) {
        ...
        if (!this._mesh || !this._sps.particles.length)
            return false;
        ...
        for (let x = ox; x < ex; x++) {
            for (let y = oy; y < ey; y++) {
                for (let z = oz; z < ez; z++) {
                    ...
                    if (texture === 0 || !iSelection.isSelected(x, y, z))
                        continue;
                    for (let i = particleCount; i < particleCount + particlesPerVoxel; i++) {
                        const particle = this._sps.particles[i];
                        particle.position.set(x + 0.5, y + 0.5, z + 0.5);
                        particle.velocity.set((Math.random() - 0.5) * speed, Math.random() * speed, (Math.random() - 0.5) * speed);
                        particle.color.r =
