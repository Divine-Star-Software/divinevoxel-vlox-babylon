...
export default function (followPosition = Vector3Like.Create()) {
    Threads.registerTask("update-debug-map-position", ([x, y, z]) => {
        followPosition.x = x;
        followPosition.y = y;
        followPosition.z = z;
    });
    Threads.registerTask("set-debug-map-canvas", (canvas) => {
        // Babylon.js setup
        const engine = new Engine(canvas);
        const scene = new Scene(engine);
        engine.setSize(canvas.width, canvas.height);
        engine.resize();
        const camera = new ArcRotateCamera("", Math.PI, 0, 800, Vector3.Zero(), scene);
