...
export class GenMap {
    ...
    init(scene) {
        ...
        this._instanceMesh.setVerticesData(VertexBuffer.PositionKind, quadData.positions);
        this._instanceMesh.setVerticesData(VertexBuffer.NormalKind, quadData.normals);
        ...
        this._instanceTool.mesh.thinInstanceSetBuffer("tileColor", colorBuffer, 4, false);
    ...
    updateTiles(location) {
        ...
        if (this._lastPosition.x == worldColumnPOSX &&
        ...
        for (let i = GenMapTile.Tiles.length - 1; i > -1; i--) {
            ...
            if (needActiveUpdate) {
                const distance = Distance3D(worldColumnPOSX, 0, worldColumnPOSZ, ...GenMapTile.Tiles[i].position);
                if (distance > 300) {
        ...
        while (this._searchQueue.length) {
            ...
            const distance = Distance3D(worldColumnPOSX, worldColumnPOSY, worldColumnPOSZ, cx, cy, cz);
            if (distance > 300)
                continue;
            const sectorKey = WorldSpaces.hash.hashXYZ(cx, cy, cz);
            if (this._visitedMap.has(sectorKey))
                continue;
            ...
            for (let i = 0; i < MooreNeighborhood2D.length; i++) {
                ...
                const columnPOS = WorldSpaces.sector.getPosition(nx, cy, nz, tempPosition);
